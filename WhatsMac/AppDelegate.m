...
@property (strong, nonatomic) ServiceContainer *serviceContainer;
...
- (void)applicationDidFinishLaunching:(NSNotification *)aNotification {
    self.serviceContainer = [[ServiceContainer alloc] init];
    self.settingsStore = self.serviceContainer.settingsStore;
    ...
    _settingsController = [[SettingsController alloc] initWithWindowNibName:@"Settings" settingsStore:self.serviceContainer.settingsStore];

    NSDate *lastUpdateDate = [[SUUpdater sharedUpdater] lastUpdateCheckDate];
    switch (self.settingsStore.updateFrequency) {
        case UpdateFrequencyAtLaunch:
            [[SUUpdater sharedUpdater] checkForUpdatesInBackground];
            break;
        case UpdateFrequencyDaily:
            if ([lastUpdateDate compare:[NSDate dateWithTimeIntervalSinceNow:-60*60*24]] == NSOrderedAscending) {
                [[SUUpdater sharedUpdater] checkForUpdatesInBackground];
            }
            break;
        case UpdateFrequencyWeekly:
            if ([lastUpdateDate compare:[NSDate dateWithTimeIntervalSinceNow:-60*60*24*7]] == NSOrderedAscending) {
                [[SUUpdater sharedUpdater] checkForUpdatesInBackground];
            }
            break;
        case UpdateFrequencyMontly:
            if ([lastUpdateDate compare:[NSDate dateWithTimeIntervalSinceNow:-60*60*24*30]] == NSOrderedAscending) {
                [[SUUpdater sharedUpdater] checkForUpdatesInBackground];
            }
            break;
...
- (void)setNotificationCount:(NSString *)notificationCount {
    if (![_notificationCount isEqualToString:notificationCount]) {
        if (self.settingsStore.notificationBadge) {
...
- (IBAction)showSettings:(id)sender {
    ...
    [_settingsController setShouldCascadeWindows:YES];
}
